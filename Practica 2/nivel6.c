...
void ctrlc(int signum){
    ...
    //sprintf(mensaje,"[ctrlc()-> recibida señal 2 (SIGINT)]\n");
    //write(2,mensaje,strlen(mensaje));
    if(jobs_list[0].pid>0){//si hay un proceso en el foreground
...
void ctrlz(int signum){
    ...
    if(jobs_list[0].pid>0){
        if(jobs_list[0].cmd!=mi_shell){//problema
            //sprintf(mensaje,"[ctrlz()->recibida señal 20 (SIGTSTP)]\n");
            //write(2,mensaje,strlen(mensaje));
            kill(jobs_list[0].pid,SIGSTOP);
            jobs_list[0].status = 'D';
            //sprintf(mensaje,"[ctrlz()->señal 19 (SIGSTOP enviada a %d (%s) por %d (%s)]\n",jobs_list[0].pid, jobs_list[0].cmd,getpid(),mi_shell);
            //write(2,mensaje,strlen(mensaje));
            ...
            //sprintf(mensaje,"[ctrlz()->señal SIGSTOP no enviado por %d (%s) debido a que su proceso en foreground es el shell]\n",jobs_list[0].pid, 
            //jobs_list[0].cmd);
            //write(2,mensaje,strlen(mensaje));
        }
    }else{
        //sprintf(mensaje,"[ctrlz()->señal SIGSTOP no enviado por %d (%s) debido a que no hay proceso en foreground]\n",jobs_list[0].pid, jobs_list[0].cmd);
        //write(2,mensaje,strlen(mensaje));
        printf("\n%c ",PROMPT);
    }
    fflush(stdout);
